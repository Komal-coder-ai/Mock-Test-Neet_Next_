...
export default function TestRunner() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Top Header Bar: sticky only on sm and above */}
      <motion.div
        className="bg-white border-b border-gray-200 sm:sticky sm:top-0 sm:z-50"
        initial={{ y: -100 }}
        animate={{ y: 0 }}
      >
      ...
      <main className="max-w-7xl mx-auto px-2 sm:px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="col-span-1 lg:col-span-9">
            ...
            >
              {subjects.map((s) => {
                const list = filteredIndicesForSubject(
                  s === "All" ? "All" : (s as any)
                );
                const answeredInSubject = list.filter(
            ...
            ${
              // Conditional styling based on whether the question is marked
              marked[currentIndex]
        ...
        `}
                  ...
                  >
                    <Bookmark
                      size={16}
                      // Change icon color based on state for better visual feedback
                      className={
                        marked[currentIndex] ? "text-white" : "text-gray-600"
                      }
                    />
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-6">
                ...
                <div className="text-center p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                  ...
                  <p className="text-2xl font-bold text-yellow-600">
                    {
                      Object.keys(marked).filter((k) => marked[Number(k)])
                        .length
                    }
                  </p>
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
                  {questions.map((_, i) => {
                    ...
                    if (isCurrent)
                      cls =
                        "bg-gradient-to-br from-blue-600 to-indigo-600 border-blue-600 text-white shadow-md";
                    else if (isMarked)
                      cls = "bg-yellow-100 border-yellow-400 text-yellow-700";
                    else if (answered)
              ...
              <AnimatePresence>
                {showSubmitModal && (
                  ...
                  >
                    ...
                    />
                    <motion.div
                      className="bg-white rounded-2xl p-8 w-[720px] max-w-full z-10 shadow-2xl"
                      initial={{ y: 20 }}
                      animate={{ y: 0 }}
                      exit={{ y: 20 }}
                    >
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-6">
                        ...
                        <div className="p-4 rounded-lg bg-blue-50 flex flex-col items-start">
                          <div className="text-3xl font-bold text-blue-700">
                            {Math.round(
                              (answeredCount / Math.max(1, total)) * 100
                            )}
                            %
                          </div>
